...
show_AST (struct AST *ast, int depth)
{
    int i;
    print_nspace (depth);
    if (   !strcmp (ast->ast_type, "ID")
        || !strcmp (ast->ast_type, "INT")
        || !strcmp (ast->ast_type, "CHAR")
        || !strcmp (ast->ast_type, "STRING")) {
...
parse_translation_unit (void)
{
    ...
    while (1) {
        switch (lookahead (1)) {
        case TK_KW_INT: case TK_KW_LONG: case TK_KW_CHAR: case TK_KW_VOID:
