...
static struct AST *parse_declarator(void) {
  ...
  switch (lookahead(1)) {
    case TK_ID:
      ast1 = create_leaf_with_lexeme(TK_ID);
      ast = add_AST(ast, 1, ast1);
      break;
    case '*':
...
static struct AST *parse_parameter_declaration(void) {
  ...
  ast = create_construct_AST(CK_PARAMETER);
...
static struct AST *parse_primary(void) {
  ...
  switch (lookahead(1)) {
    ...
    case TK_STRING:
    case TK_ID:
      ast1 = create_leaf_with_lexeme(lookahead(1));
      ast = add_AST(ast, 1, ast1);
      break;
    case '(':
      ast1 = create_final_word('(');
...
static struct AST *parse_statement(void) {
  ...
  switch (lookahead(1)) {
    ...
    case TK_KW_GOTO:
      ...
      ast2 = create_leaf_with_lexeme(TK_ID);
      ast3 = create_final_word(';');
      ast = add_AST(ast, 3, ast1, ast2, ast3);
      break;
    case TK_KW_RETURN:
