...
static char *library_funcs[] = {
    ...
    "printf", "malloc", "random", "srandom", "atol", "exit", NULL};
    ...
    case NS_GLOBAL:
      ...
      if (sym->type->kind == TYPE_KIND_FUNCTION) {
        if (is_library_func(sym->name)) {
          emit_code(ast, "\tmovq    %s@GOTPCREL(%%rip), %%rax\n", sym->name);
        } else {
          emit_code(ast, "\tleaq    %s(%%rip), %%rax\n", sym->name);
        }
        emit_code(ast, "\tpushq   %%rax\n");
      } else {
        emit_code(ast, "\tpushq   %s(%%rip)\n", sym->name);
      }
      break;
...
static void codegen_exp_left(struct AST *ast) {
  if (!strcmp(ast->ast_type, "AST_expression_unary")) {
...
static void codegen_exp_funcall(struct AST *ast_func) {
  ...
  for (int i = 1; i <= ARRAY_SIZE(regs); i++) {
  ...
  for (int i = 1; i <= narg && i <= ARRAY_SIZE(arg_regs); i++) {
  ...
  for (int i = ARRAY_SIZE(regs); i >= 1; i--) {
    emit_code(ast_func, "\tpopq   %s\n", regs[i - 1]);
  }
...
static void codegen_exp(struct AST *ast) {
  if (!strcmp(ast->ast_type, "AST_expression_int") ||
    ...
    if (ast->child[0]->type->kind == TYPE_KIND_POINTER) {
      if (ast->child[1]->type->kind == TYPE_KIND_POINTER) {
        // pointer - pointer
        codegen_exp(ast->child[0]);
        codegen_exp(ast->child[1]);
        emit_code(ast,
...
static void codegen_stmt(struct AST *ast_stmt) {
  if (!strcmp(ast_stmt->ast_type, "AST_statement_exp")) {
    ...
    int while_label_id = label_id;
    label_id += 2;
    emit_code(ast_stmt, "LBB0_%d:\n", while_label_id);
    codegen_exp(ast_stmt->child[0]);
    emit_code(ast_stmt,
              "\tpopq    %%rax\n\tcmpq    $0,   %%rax\n\tje    LBB0_%d\n",
              while_label_id + 1);
    codegen_stmt(ast_stmt->child[1]);
    emit_code(ast_stmt, "\tjmp    LBB0_%d\nLBB0_%d:\n", while_label_id,
              while_label_id + 1);
